...
func (c *Client) CheckSessionContext(ctx context.Context) (*api.Session, time.Duration, error) {
	...
	if c.config.Username == "" {
		if cachedUser, err := c.sessionStore.UsernameContext(ctx); err == nil && cachedUser != "" {
			c.config.Username = cachedUser
		}
	}
...
func (c *Client) ensureUsername() error {
	if c.config.Username == "" {
		if cachedUser, err := c.sessionStore.Username(); err == nil && cachedUser != "" {
			c.config.Username = cachedUser
			return nil
		}
