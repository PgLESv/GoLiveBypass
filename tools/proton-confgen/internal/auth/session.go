...
func (s *SessionStore) LoadContext(ctx context.Context, username string) (*api.Session, time.Duration, error) {
	...
	err := s.withSessionLockContext(ctx, func() error {
		...
		if username != "" && !strings.EqualFold(strings.TrimSpace(savedSession.Username), strings.TrimSpace(username)) {
			return nil
		}
...
func (s *SessionStore) LoadSaved(username string) (*SavedSession, time.Duration, error) {
	...
	var timeUntilExpiry time.Duration
	err := s.withSessionLockContext(context.Background(), func() error {
		data, err := s.readPayloadLocked()
		if err != nil {
			return err
		}
