...
export function wireSockSearchRoots(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  if (env.ProgramFiles === undefined && env.ProgramW6432 === undefined && env["ProgramFiles(x86)"] === undefined) {
    candidates.push("C:\\Program Files");
  }
  const programFiles = candidates.filter((dir): dir is string => Boolean(dir));
