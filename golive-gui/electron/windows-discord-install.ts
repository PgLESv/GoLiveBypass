...
export function findWindowsDiscordInstall(
  ...
  let directExePath: string | undefined;
  try {
    const entries = readdirSync(rootPath);
    const directMatch = entries.find((entry) =>
      directExeNames.some((name) => name.toLowerCase() === entry.toLowerCase()) && existsSync(path.join(rootPath, entry)),
    );
    if (directMatch) {
      directExePath = path.join(rootPath, directMatch);
    }
  } catch {
    directExePath = directExeNames
