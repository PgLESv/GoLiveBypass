...
vi.mock("../electron/updater-replace", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../electron/updater-replace")>();
  return {
    ...actual,
    cleanupOldExe: vi.fn(),
    spawnWindowsUpdateHelper: state.spawn,
  };
});
...
it("baixa, persiste identidade e aplica GUI com digest e tamanho corretos", async () => {
  ...
  expect(state.spawn).toHaveBeenCalledWith(process.env.PORTABLE_EXECUTABLE_FILE, saved.downloaded, expect.any(String));
...
it("restaura e aplica o portable valido apos reiniciar", async () => {
  ...
  const controller = updater.setupUpdater(() => null, () => false, () => "beta")!;
  expect(await controller.applyPendingUpdate()).toBe(true);
  expect(state.spawn).toHaveBeenCalledWith(pending.current, pending.downloaded, expect.any(String));
});
