...
describe("preflight Linux", () => {
  ...
  it.runIf(process.platform === "linux")("carrega o modulo WireGuard antes de fechar o Discord e aborta sem namespace em falha", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "../standalone/golivebypass-standalone.sh"), "utf8");
  ...
  it.runIf(process.platform === "linux")("não fecha o Discord quando a autorização falha e mantém a ordem quando aceita", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "../standalone/golivebypass-standalone.sh"), "utf8");
  ...
  it.runIf(process.platform === "linux")("limpa recursos legados depois da autorizacao e antes de fechar o Discord", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "../standalone/golivebypass-standalone.sh"), "utf8");
    const installStart = source.indexOf('FOUND="$(escolher_alvos patchear)"');
    const installEnd = source.indexOf("\nwhile IFS='|' read", installStart);
    expect(installStart).toBeGreaterThanOrEqual(0);
  ...
  it.runIf(process.platform === "linux")("mantem o rollback pendente ate o Discord iniciar e o limpa depois da confirmacao", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "../standalone/golivebypass-standalone.sh"), "utf8");
  ...
  it.runIf(process.platform === "linux")("seleciona runuser ou setpriv sem sudo e preserva a entrada no namespace", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "../standalone/golivebypass-standalone.sh"), "utf8");
  ...
  it.runIf(process.platform === "linux")("instala apenas comandos ausentes com argv pacman fixo e verifica o resultado", () => {
  ...
  it.runIf(process.platform === "linux")("é idempotente e não chama o gerenciador quando tudo já existe", () => {
  ...
  it.runIf(process.platform === "linux")("gera o plano de pacotes correto para cada família suportada", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "../standalone/golivebypass-standalone.sh"), "utf8");
  ...
  it.runIf(process.platform === "linux")("exibe um comando de reparo especifico sem upgrade global", () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "../standalone/golivebypass-standalone.sh"), "utf8");
  ...
  it.runIf(process.platform === "linux")("confirma o processo no namespace, usa readonly sem prompt e faz rollback se ele sumir", async () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), "../standalone/golivebypass-standalone.sh"), "utf8");
