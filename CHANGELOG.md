...
- Causa confirmada: no Windows PowerShell 5.1 a primeira linha que um processo nativo escreve em **stderr** vira erro terminativo enquanto `$ErrorActionPreference='Stop'`, mesmo com `2>&1` (o mesmo caso que já derrubava o probe do corepack logo acima). O pnpm escreve o próprio banner (`$ node scripts/runInstaller.mjs …`) em stderr e o Equilotl — injetor atual do Equicord, baixado e executado por `pnpm inject` — loga **tudo** em stderr; `Invoke-Pnpm` morria nessa primeira linha e a injeção não começava.
- Correção: `Invoke-Pnpm` roda o processo nativo com `ErrorActionPreference='Continue'` (restaurado no `finally`), junta o stderr na saída capturada e continua lendo `$LASTEXITCODE`; o código de saída segue diagnóstico e a pós-condição (`Test-TargetInjectedFromCheckout`) segue autoridade. O ponto de injeção deixou de depender de `2>&1` para guardar o detalhe.
- Evidência: [pendente — rodada de instalação na VM `win11`].
