...
- Evidência (VM `win11`, 19/09, pnpm 11.24.0, checkout Equicord real, scripts gerados do código): o instalador do `main` reproduz o relato 1:1 (`exit=-1`, `excecao=` o banner do pnpm, injetor sem rodar); o corrigido segue com a injeção (`Downloading EquilotlCli.exe…`), captura a saída real do injetor (14 itens) e deixa a pós-condição decidir. `tests/test-error-handling.ps1` passou 85/85 com o instalador corrigido e 84/85 com o do `main` (falha só na captura do stderr). O stub da regressão passou a usar `-EncodedCommand`: com `-Command`, o argumento com espaços chega fatiado ao processo nativo no PS 5.1 e o teste media outra coisa.
