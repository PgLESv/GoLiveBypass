...
for f in index.tsx native.ts update-channel.ts update-security.ts stability.ts vpn-controller.ts vpn-proton.ts vpn-types.ts vpn-snapshot.ts vpn-snapshot-worker.ts vpn-windows.ts manifest.json bin/win32-x64/proton-confgen.exe; do
    if [ -f "$target/$f" ]; then
